...
static double xs_range(double lo, double hi) {
    double t = (double)xs_next() / (double)UINT32_MAX; /* [0,1) */
    return lo + t * (hi - lo);
}

cairo_surface_t* create_brushed_aluminium(int w, int h, uint32_t seed) {
    const int ss = 2;
    cairo_surface_t* surf = cairo_image_surface_create(CAIRO_FORMAT_ARGB32, w*ss, h*ss);
    cairo_t* cr = cairo_create(surf);

    cairo_pattern_t* base = cairo_pattern_create_linear(0, 0, 0, h*ss);
    ...
    for (int y = 0; y < h*ss; ++y) {
        double b = xs_range(-0.05, 0.05);
        double a = xs_range(0.01, 0.12);
        cairo_set_source_rgba(cr, 0.55+b, 0.55+b, 0.57+b, a);
        cairo_set_line_width(cr, 1.0);
        cairo_move_to(cr, 0, y + 0.5);
        cairo_line_to(cr, w*ss, y + 0.5);
        cairo_stroke(cr);
    }

    cairo_pattern_t* glare = cairo_pattern_create_linear(0, 0, w*ss, h*ss);
    ...
    cairo_pattern_t* vig = cairo_pattern_create_radial(
        w*ss*0.5, h*ss*0.5, h*ss*0.2,
        w*ss*0.5, h*ss*0.5, h*ss*0.75);
    ...
    cairo_surface_t* texture = cairo_image_surface_create(CAIRO_FORMAT_ARGB32, w, h);
    cairo_t* fcr = cairo_create(texture);
    cairo_scale(fcr, 1.0/ss, 1.0/ss);
    cairo_set_source_surface(fcr, surf, 0, 0);
    cairo_pattern_set_filter(cairo_get_source(fcr), CAIRO_FILTER_GOOD);
    cairo_paint(fcr);
    cairo_destroy(fcr);
...
void shade_bg_color(Widget_t* w, float shade, float alpha) {
    Colors *c = get_color_scheme(w, NORMAL_);
    cairo_set_source_rgba(w->crb, c->bg[0]*shade, c->bg[1]*shade, c->bg[2]*shade, alpha);
}
...
void draw_ir_elem(void *w_, void* user_data) {
    ...
    if (!w->image) w->image = create_brushed_aluminium(width_t, height_t, 0xA10A11u);
    ...
    if (height != height_t || width != width_t) {
        ...
        w->image = NULL;
        w->image = create_brushed_aluminium(width_t, height_t, 0xA10A11u);
    }
    ...
    setFrameColour(w,  10 * w->scale.rcscale_x * w->app->hdpi, 10 * w->scale.rcscale_y * w->app->hdpi,
        w->width-20 * w->scale.rcscale_x * w->app->hdpi, w->height-20 * w->scale.rcscale_y * w->app->hdpi);
    cairo_stroke (w->crb);
...
static void draw_window(void *w_, void* user_data) {
    ...
    int width_t = metrics.width;
    int height_t = metrics.height;
    if (!w->image) w->image = create_brushed_aluminium(width_t, height_t, 0xC10A11u);
    int width = cairo_image_surface_get_width(w->image);
    int height = cairo_image_surface_get_height(w->image);
    if (height != height_t || width != width_t) {
        cairo_surface_destroy(w->image);
        w->image = NULL;
        w->image = create_brushed_aluminium(width_t, height_t, 0xC10A11u);
    }

    cairo_set_source_surface(w->crb, w->image, 0, 0);
    cairo_paint (w->crb);
    cairo_set_source_rgba(w->crb, 0.083, 0.083, 0.083, 0.5);    
    cairo_paint (w->crb);
...
static void draw_my_vswitch(void *w_, void* user_data) {
    ...
    surface_set_font_from_ttf(wid->crb, LDVAR_FONT(RobotoCondensedBold_ttf),
                                        LDLEN_FONT(RobotoCondensedBold_ttf));
    if (!state) {
        widget_hide(ui->widget[6]);
        widget_hide(ui->widget[7]);
        widget_show(ui->widget[0]);
        widget_show(ui->widget[1]);        
        use_fg_color_scheme(wid, INSENSITIVE_);
    } else {
        widget_hide(ui->widget[0]);
        widget_hide(ui->widget[1]);
        widget_show(ui->widget[6]);
        widget_show(ui->widget[7]);        
